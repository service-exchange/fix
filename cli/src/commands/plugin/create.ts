...
export default class Create extends Command {
  ...
  static description = "Create a new telecms plugin";
  ...
  async run(): Promise<void> {
    ...
    if (Number(name)) {
      this.log(
        "\x1b[41m%s\x1b[0m",
        "Error : Plugin Display name can not be a number"
      );
      process.exit(1);
    }
    ...
    if (
      !(
        ...
        fs.existsSync(defaultTemplates)
      )
    ) {
      this.log(
        "\x1b[41m%s\x1b[0m",
        "Error : plugins, docs or plugins/_templates directory missing, make sure that you are runing this command in Tele-CMS directory"
      );
      process.exit(1);
    }
    ...
    this.log(
      "\x1b[42m",
      "\x1b[30m",
      `Plugin: ${args.plugin_name} created successfully`,
      "\x1b[0m"
    );
