...
describe('organizations controller', () => {
  ...
  describe('list organization users', () => {
    ...
    it('should list organization users', async () => {
      const userData = await createUser(app, { email: 'admin@service.exchange' });
    ...
    describe('create organization', () => {
      ...
      it('should throw error if name is empty', async () => {
        const { user } = await createUser(app, { email: 'admin@service.exchange' });
        const response = await request(app.getHttpServer())
          .post('/api/organizations')
          .send({ name: '' })
          .set('Authorization', authHeaderForUser(user));
    ...
    describe('update organization', () => {
      it('should change organization params if changes are done by admin', async () => {
        ...
        const response = await request(app.getHttpServer())
          ...
          .send({ name: 'new name', domain: 'service.exchange', enableSignUp: true })
          .set('Authorization', authHeaderForUser(user));

        expect(response.statusCode).toBe(200);
        await organization.reload();
        expect(organization.name).toBe('new name');
        expect(organization.domain).toBe('service.exchange');
      ...
      it('should not change organization params if changes are not done by admin', async () => {
        const { organization } = await createUser(app, { email: 'admin@service.exchange' });
        const developerUserData = await createUser(app, {
          email: 'developer@service.exchange',
          groups: ['all_users'],
          organization,
        });
        const response = await request(app.getHttpServer())
