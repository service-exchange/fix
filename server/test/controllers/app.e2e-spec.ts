...
describe('Authentication', () => {
  ...
  describe('Single organization', () => {
    ...
    it('should create new users and organization', async () => {
      const response = await request(app.getHttpServer()).post('/api/signup').send({ email: 'test@service.exchange' });
      expect(response.statusCode).toBe(201);

      const user = await userRepository.findOneOrFail({
        where: { email: 'test@service.exchange' },
    ...
    describe('Single organization operations', () => {
      beforeEach(async () => {
        current_organization = (await createUser(app, { email: 'admin@service.exchange' })).organization;
      });
      ...
      it('authenticate if valid credentials', async () => {
        await request(app.getHttpServer())
          ...
          .send({ email: 'admin@service.exchange', password: 'password' })
          .expect(201);
      });
      ...
      it('throw 401 if user is archived', async () => {
        await createUser(app, { email: 'user@service.exchange', status: 'archived' });

        await request(app.getHttpServer())
          .post('/api/authenticate')
          .send({ email: 'user@service.exchange', password: 'password' })
          .expect(401);

        const adminUser = await userRepository.findOneOrFail({
          email: 'admin@service.exchange',
        });
      ...
      it('throw 401 if user is invited', async () => {
        await createUser(app, { email: 'user@service.exchange', status: 'invited' });
      ...
      it('throw 401 if invalid credentials', async () => {
        await request(app.getHttpServer())
          .post('/api/authenticate')
          .send({ email: 'amdin@service.exchange', password: 'pwd' })
          .expect(401);
      });
  ...
  describe('Multi organization', () => {
    ...
    describe('sign up enabled and authorization', () => {
      ...
      it('throw 401 if user is archived', async () => {
        const { orgUser } = await createUser(app, { email: 'user@service.exchange', status: 'archived' });
      ...
      it('throw 401 if user is invited', async () => {
        const { orgUser } = await createUser(app, { email: 'user@service.exchange', status: 'invited' });
      ...
      it('throw 401 if invalid credentials, maximum retry limit reached error after 5 retries', async () => {
        await request(app.getHttpServer())
          .post('/api/authenticate')
          .send({ email: 'admin@service.exchange', password: 'pwd' })
          .expect(401);
  ...
  describe('POST /api/forgot-password', () => {
    ...
    it('should set token and send email', async () => {
      ...
      const response = await request(app.getHttpServer())
        ...
        .send({ email: 'admin@service.exchange' });

      expect(response.statusCode).toBe(201);

      const user = await getManager().findOne(User, {
        where: { email: 'admin@service.exchange' },
      });
  ...
  describe('POST /api/reset-password', () => {
    ...
    it('should reset password', async () => {
      ...
      await request(app.getHttpServer())
        .post('/api/authenticate')
        .send({ email: 'admin@service.exchange', password: 'new_password' })
        .expect(201);
    });
  });
  ...
  describe('POST /api/set-password-from-token', () => {
    ...
    it('should allow users to signup for single organization only once', async () => {
      ...
      await request(app.getHttpServer()).post('/api/signup').send({ email: 'invited@service.exchange' }).expect(201);
      await request(app.getHttpServer()).post('/api/signup').send({ email: 'newinvited@service.exchange' }).expect(406);
    });

    it('should allow users to signup - Multi-Workspace', async () => {
      await request(app.getHttpServer()).post('/api/signup').send({ email: 'invited@service.exchange' }).expect(201);
      await request(app.getHttpServer()).post('/api/signup').send({ email: 'newinvited@service.exchange' }).expect(201);
      await request(app.getHttpServer()).post('/api/signup').send({ email: 'newinvited1@service.exchange' }).expect(201);
    });
    ...
    it('should allow users to sign up and setup account if already invited to an organization but not setup the account', async () => {
      ...
      await request(app.getHttpServer())
        ...
        .set('Authorization', authHeaderForUser(adminUser))
        .send({ email: 'invited@service.exchange' })
        .expect(201);
      ...
      const invitedUserDetails = await getManager().findOneOrFail(User, { where: { email: 'invited@service.exchange' } });
      ...
      expect(response.statusCode).toBe(201);
      const updatedUser = await getManager().findOneOrFail(User, { where: { email: 'invited@service.exchange' } });
      expect(updatedUser.firstName).toEqual('signupuser');
      expect(updatedUser.lastName).toEqual('user');
  ...
  describe('POST /api/accept-invite', () => {
    describe('Multi-Workspace Enabled', () => {
      ...
      it('should allow users to accept invitation when Multi-Workspace is enabled', async () => {
        const userData = await createUser(app, {
          email: 'organizationUser@service.exchange',
          status: 'invited',
        });
