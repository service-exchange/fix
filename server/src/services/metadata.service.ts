...
export class MetadataService {
  ...
  async finishInstallation(metadata: any, installedVersion: string, name: string, email: string, org: string) {
    return await got('https://hub.service.exchange/subscribe', {
      method: 'post',
      json: {
        id: metadata.id,
  ...
  async sendTelemetryData(metadata: Metadata) {
    ...
    return await got('https://hub.service.exchange/telemetry', {
      method: 'post',
      json: {
        id: metadata.id,
        ...
        telecms_version: globalThis.TELECMS_VERSION,
  ...
  async checkForUpdates(metadata: Metadata) {
    const installedVersion = globalThis.TELECMS_VERSION;
    const response = await got('https://hub.service.exchange/updates', {
      method: 'post',
    });
