...
globalThis.TELECMS_VERSION = fs.readFileSync('./.version', 'utf8').trim();
...
async function bootstrap() {
  ...
  const host = new URL(process.env.TELECMS_HOST);
  ...
  await app.listen(port, '0.0.0.0', function () {
    const telecmsHost = configService.get<string>('TELECMS_HOST');
    console.log(`Ready to use at ${telecmsHost} 🚀`);
  });
}

// Bootstrap global agent only if TELECMS_HTTP_PROXY is set
if (process.env.TELECMS_HTTP_PROXY) {
  process.env['GLOBAL_AGENT_HTTP_PROXY'] = process.env.TELECMS_HTTP_PROXY;
