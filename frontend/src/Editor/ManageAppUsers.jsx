...
class ManageAppUsers extends React.Component {
  ...
  render() {
    ...
    const appId = app.id;
    const appLink = `${window.public_config?.TELECMS_HOST}/applications/`;
    const shareableLink = appLink + (this.props.slug || appId);
    const slugButtonClass = isSlugVerificationInProgress ? '' : slugError !== null ? 'is-invalid' : 'is-valid';
    const embeddableLink = `<iframe width="560" height="315" src="${appLink}${this.props.slug}" title="Tele-CMS app - ${this.props.slug}" frameborder="0" allowfullscreen></iframe>`;
