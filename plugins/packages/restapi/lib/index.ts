...
import { QueryError, QueryResult, QueryService, cleanSensitiveData } from '@service-exchange/common';
    ...
    if (requiresOauth) {
      ...
      if (!tokenData) {
        ...
        const authUrl = new URL(
          `${sourceOptions['auth_url']}?response_type=code&client_id=${sourceOptions['client_id']}&redirect_uri=${telecmsHost}/oauth2/authorize&scope=${sourceOptions['scopes']}`
        );
