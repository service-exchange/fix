import { QueryError, QueryResult, QueryService } from '@service-exchange/common';
...
export default class SlackQueryService implements QueryService {
  authUrl(): string {
    ...
    const telecmsHost = process.env.TELECMS_HOST;
    return `https://slack.com/oauth/v2/authorize?response_type=code&client_id=${clientId}&redirect_uri=${telecmsHost}/oauth2/authorize`;
  }
  ...
  async accessDetailsFrom(authCode: string): Promise<object> {
    ...
    const telecmsHost = process.env.TELECMS_HOST;
    const redirectUri = `${telecmsHost}/oauth2/authorize`;
