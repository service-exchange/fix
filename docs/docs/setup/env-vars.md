...
Both the Tele-CMS server and client requires some environment variables to start running.

## Tele-CMS server

#### Tele-CMS host ( required )

| variable     | description                                                     |
| ------------ | --------------------------------------------------------------- |
| TELECMS_HOST | the public URL of Tele-CMS client ( eg: <https://app.service.exchange> )  |

#### Lockbox configuration ( required )

Tele-CMS server uses lockbox to encrypt datasource credentials. You should set the environment variable `LOCKBOX_MASTER_KEY` with a 32 byte hexadecimal string.

#### Application Secret ( required )

Tele-CMS server uses a secure 64 byte hexadecimal string to encrypt session cookies. You should set the environment variable `SECRET_KEY_BASE`.
...
Tele-CMS server uses PostgreSQL as the database.
...
If Tele-CMS is hosted on a domain subpath, you can set the environment variable `SUB_PATH` to support it.
...
Tele-CMS uses SMTP services to send emails ( Eg: invitation email when you add new users to your workspace ).

| variable           | description                               |
| ------------------ | ----------------------------------------- |
| DEFAULT_FROM_EMAIL | from email for the email fired by Tele-CMS |
...
If your Tele-CMS installation requires Slack as a data source, you need to create a Slack app and set the following environment variables:
...
If your Tele-CMS installation needs access to data sources such as Google sheets, you need to create OAuth credentials from Google Cloud Console.
...
If your Tele-CMS installation requires `Maps` widget, you need to create an API key for Google Maps API.
...
| variable           | description                                                 |
| ------------------ | ----------------------------------------------------------- |
| TELECMS_SERVER_URL | the URL of Tele-CMS server ( eg: <https://server.service.exchange> ) |
...
Tele-CMS needs to be configured for custom CA certificate to be able to trust and establish connection over https. This requires you to configure an additional env var `NODE_EXTRA_CA_CERTS` to have absolute path to your CA certificates. This file named `cert.pem` needs to be in PEM format and can have more than one certificates.

| variable            | description                                                       |
| ------------------  | ----------------------------------------------------------------- |
| NODE_EXTRA_CA_CERTS | absolute path to certifcate PEM file ( eg: /Tele-CMS/ca/cert.pem ) |

#### Disable telemetry ( optional )

Pings our server to update the total user count every 24 hours. You can disable this by setting the value of `DISABLE_TELECMS_TELEMETRY` environment variable to `true`. This feature is enabled by default.
...
This could be used to for local development, it will set the server url like so: `http://localhost:<TELECMS_SERVER_PORT>`

| variable            | description                             |
|---------------------|-----------------------------------------|
| TELECMS_SERVER_PORT | the port of Tele-CMS server ( eg: 3000 ) |
...
| variable           | description                                                   |
| ------------------ | -----------------------------------------------------------   |
| ASSET_PATH         | the asset path for the website ( eg: <https://app.service.exchange/>) |

#### Serve client as a server end-point ( optional )

By default the client build will be done to be served with Tele-CMS server.
